...
public class GoneWrongController : MonoBehaviour
{
    ...
    [SerializeField] [Range(1,15)] float difficultyCurb; // Where difficultyCurb is c in equation, y = (-2 ^ (d-cx)) + 1
    [SerializeField] [Range(-6,6)] float difficultyStartOffset; // Where difficultyOffset is d in equation, y = (-2 ^ (d-cx)) + 1
    [SerializeField] [Range(-6,6)] float difficultyMaxOffset; // Where difficultyOffset is d in equation, y = (-2 ^ (d-cx)) + 1
    ...
    public void AddNewLayerToChooseFrom(GameObject newLayerToGoWrong)
    {
        ...
        if (goingWrong) 
        {
            ...
            curbRes = -MathF.Pow(2.0f, difficultyStartOffset - (difficultyCurb * ((float)currentLayer / maxLayers))) + difficultyMaxOffset;
        }
    }
